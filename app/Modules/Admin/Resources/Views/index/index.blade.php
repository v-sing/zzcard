...
<head>
    <meta charset="UTF-8">
    <title>{{config('platform.website')}}</title>
...
<div class="container">
    <div class="logo">
        <a>{{config('platform.website')}}</a></div>
    {{--<div class="left_open">
    ...
    <ul class="layui-nav right" lay-filter="">
        <li class="layui-nav-item">
            ...
            <dl class="layui-nav-child">
                <!-- 二级菜单 -->
                <dd>
                    <a onclick="xadmin.open('个人信息','/admin/admin/password',500,400)">修改密码</a>
                </dd>
                <dd>
                    <a  class="logout">退出</a></dd>
            </dl>
        </li>
...
<div class="left-nav">
    <div id="side-nav">
            @foreach($menu as $v)
                ...
                <ul class="ul_nav"  id="ul_nav_{{$v['id']}}" style="display: none">
                    @foreach($v['children'] as $vv1)
                        <li>
                            @isset($vv1['children'])
                                ...
                                <ul class="sub-menu">
                                    @foreach($vv1['children'] as $v2)
                                            ...
                                            <li>
                                                ...
                                                <ul class="sub-menu">
                                                    @foreach($v2['children'] as $v3)
                                                            ...
                                                            <li>
                                                                ...
                                                                <ul class="sub-menu">
                                                                    @foreach($v3['children'] as $v4)
                                                                        <li>
                                                                            <a style="padding: 12px 15px 12px 60px;" class="add_tab" onclick="xadmin.add_tab('{{$v4["menu_name"]}}','{{$v4["url"]}}',true)">
                                                        ...
                                                        @else
                                                            <li>
                                                                <a onclick="xadmin.add_tab('{{$v3["menu_name"]}}','{{$v3["url"]}}',true)" class="add_tab">
                                                                    <i class="iconfont">{!! $v3['icon'] !!}</i>
                                        ...
                                        @else
                                            <li>
                                                <a onclick='xadmin.add_tab("{{$v2['menu_name']}}","{{$v2['url']}}",true)' class="add_tab">
                                                    <i class="iconfont">{!! $v2['icon'] !!}</i>
                            ...
                            @else
                                <a onclick="xadmin.add_tab('{{$vv1["menu_name"]}}','{{$vv1["url"]}}',true)">
                ...
                @else
                <a onclick="xadmin.add_tab('{{$v["menu_name"]}}','{{$v["url"]}}',true)" id="a_nav_{{$v['id']}}" style="display: none"></a>
                @endisset
            @endforeach
    </div>
</div>
    ...
    <div class="layui-tab tab" lay-filter="xbs_tab" lay-allowclose="false">
        ...
        <div class="layui-tab-content">
            <div class="layui-tab-item layui-show">
                <iframe src='{{config('platform.welcome')}}' frameborder="0" scrolling="yes" class="x-iframe"></iframe>
            </div>
        </div>
        <div id="tab_show"></div>
    </div>
